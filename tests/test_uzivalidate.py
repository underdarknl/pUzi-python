...
class TestUziValidator(unittest.TestCase):
    ...
    def test_strict_ca(self):
        self.checkUser("mock-007-strict-ca-check.cert", "CA OID not UZI register Care Provider or named employee")

    def test_incorrect_version(self):
        self.checkUser("mock-008-invalid-version.cert", "UZI version not 1")

    def test_not_allowed_type(self):
        self.checkUser("mock-009-invalid-types.cert", "UZI card type not allowed", [UZI_TYPE_CARE_PROVIDER])

    def test_not_allowed_role(self):
        self.checkUser("mock-010-invalid-roles.cert", "UZI card role not allowed", [UZI_TYPE_CARE_PROVIDER], [UZI_ROLE_NURSE])
