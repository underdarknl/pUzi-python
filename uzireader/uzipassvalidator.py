...
class UziPassValidator:
    ...
    def validate(self, user: UziPassUser):
        ...
        oidca = user['OidCa']
        if self.strict_ca and oidca != OID_CA_CARE_PROVIDER and oidca != OID_CA_NAMED_EMPLOYEE:
            raise UziException("CA OID not UZI register Care Provider or named employee")
